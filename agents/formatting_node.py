...
역할:
    LLM 판단 없이 Markdown 초안을 ReportLab으로 PDF 변환하는 변환 전용 Node.
    Agent로 분류하지 않는 이유: 검색·생성·판단 없이 형식 변환만 수행하기 때문이다.

종료 흐름:
    PDF 생성 완료 후 final_report_path를 state에 저장하고 Supervisor로 복귀한다.
    Supervisor가 final_report_path를 확인하고 워크플로우를 종료한다.

한글 폰트:
    시스템에 설치된 폰트 경로를 순서대로 탐색하며, 찾지 못하면 Helvetica로 fallback한다.
"""
import os
import re
...
COLOR_SECONDARY = HexColor("#2E86AB")
COLOR_ACCENT    = HexColor("#E84855")
COLOR_LIGHT     = HexColor("#F5F5F5")
COLOR_BORDER    = HexColor("#CCCCCC")


def _register_fonts() -> str:
    """시스템 한글 폰트를 탐색하여 등록한다. 찾지 못하면 Helvetica를 반환한다."""
    font_paths = [
...
def _get_styles(font_name: str) -> dict:
    ...
    return {
        "title": ParagraphStyle(
            ...
            spaceAfter=12, spaceBefore=6, leading=28,
        ),
        ...
        "h1": ParagraphStyle(
            "H1",
            fontName=font_name, fontSize=14, textColor=COLOR_PRIMARY,
            spaceBefore=18, spaceAfter=8, borderPad=6, leading=20, leftIndent=0,
        ),
        "h2": ParagraphStyle(
            "H2",
            fontName=font_name, fontSize=12, textColor=COLOR_SECONDARY,
            spaceBefore=12, spaceAfter=6, leading=18, leftIndent=10,
        ),
        "h3": ParagraphStyle(
            "H3",
            fontName=font_name, fontSize=11, textColor=HexColor("#333333"),
            spaceBefore=8, spaceAfter=4, leading=16, leftIndent=20,
        ),
        "body": ParagraphStyle(
            "Body",
            fontName=font_name, fontSize=10, textColor=black,
            spaceAfter=6, leading=16, leftIndent=10, rightIndent=10,
            alignment=TA_JUSTIFY,
        ),
        "bullet": ParagraphStyle(
            "Bullet",
            fontName=font_name, fontSize=10, textColor=black,
            spaceAfter=4, leading=15, leftIndent=20, bulletIndent=10,
        ),
        "summary_box": ParagraphStyle(
            "SummaryBox",
            fontName=font_name, fontSize=10,
            textColor=HexColor("#1E3A5F"), leading=16, leftIndent=10,
        ),
        "reference": ParagraphStyle(
            "Reference",
            fontName=font_name, fontSize=9,
            textColor=HexColor("#555555"), spaceAfter=4, leading=13, leftIndent=10,
        ),
        "trl_note": ParagraphStyle(
            "TRLNote",
            fontName=font_name, fontSize=9,
            textColor=HexColor("#8B0000"), leading=13, leftIndent=10, spaceBefore=4,
        ),
    }


def _parse_draft(draft: str) -> List[Tuple[str, str]]:
    """Markdown 초안을 (type, content) 튜플 리스트로 파싱한다."""
    elements = []
...
def _make_cover_page(styles: dict, font_name: str) -> List:
    ...
    notice_para = Paragraph(
        "⚠ 주의: TRL 4~6 구간의 추정값은 특허 출원 패턴, 학회 발표 빈도, "
        "채용공고 키워드 등 간접 지표 기반이며 실제와 다를 수 있습니다.",
        ParagraphStyle("Notice", fontName=font_name, fontSize=9,
                       textColor=HexColor("#856404"), leading=14)
    )
    notice_table = Table([[notice_para]], colWidths=[15 * cm])
...
def _add_table_from_markdown(story: List, line: str, font_name: str) -> None:
    """Markdown 테이블 행을 ReportLab Table로 변환하여 story에 추가한다."""
    ...
    table_data = [[
        Paragraph(c, ParagraphStyle("TableCell", fontName=font_name, fontSize=9, leading=13))
        for c in safe_cells
    ]]
...
def generate_pdf(draft: str, output_path: str) -> str:
    """보고서 초안을 PDF로 변환하여 output_path에 저장한다."""
    ...
    for elem_type, content in parsed:
        ...
        if elem_type == "h1":
            if "SUMMARY" in content.upper():
                in_summary = True
                story.append(Paragraph(f"■ {safe}", styles["h1"]))
            ...
            if in_summary:
                summary_content.append(f"<b>{safe}</b>")
            else:
                story.append(Paragraph(f"▶ {safe}", styles["h2"]))

        elif elem_type == "h3":
            if in_summary:
                summary_content.append(safe)
            else:
                story.append(Paragraph(f"◆ {safe}", styles["h3"]))

        elif elem_type == "bullet":
            if in_summary:
                summary_content.append(f"• {safe}")
            else:
                story.append(Paragraph(f"• {safe}", styles["bullet"]))

        elif elem_type == "trl_note":
            story.append(Paragraph(
                safe if safe.startswith("※") else f"※ {safe}",
                styles["trl_note"]
            ))
    ...
    def add_page_number(canvas, doc):
        ...
        canvas.drawString(2 * cm, 1.2 * cm, "CONFIDENTIAL - 내부 배포용")
        canvas.restoreState()
...
def run_formatting_node(state: AgentState) -> AgentState:
    ...
    try:
        ...
        state["final_report_path"] = txt_path
